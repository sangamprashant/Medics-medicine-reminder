...
const AddReminder = () => {
  ...
  const saveReminder = async () => {
    ...
    if (!dose || isNaN(Number(dose)) || Number(dose) <= 0) {
      Alert.alert("Validation Error", "Please enter a valid dose.");
      return;
    }
    if (!frequency || isNaN(Number(frequency)) || ![1, 2, 3].includes(Number(frequency))) {
      Alert.alert("Validation Error", "Please select a valid frequency (1, 2, or 3 times/day).");
      return;
    }
    ...
    if (!consume || !["1", "2"].includes(consume)) {
      Alert.alert("Validation Error", "Please select when to consume the medicine (Before or After meal).");
      return;
    }
